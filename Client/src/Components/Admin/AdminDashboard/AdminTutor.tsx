...
import { adminListAllTutors } from "../../../Utils/config/axios.GetMethods";
import { adminBlockTutor, adminUnblockTutor } from "../../../Utils/config/axios.PutMethod";
...
function AdminTutor() {
  ...
  const userStatus = async (tutor: Tutor) => {
    console.log("Button Clicked");
    try {
      const updatedStatus = !tutor.isBlocked;
      await (updatedStatus ? adminBlockTutor(tutor._id) : adminUnblockTutor(tutor._id));

      tutor.isBlocked = updatedStatus;
      ...
      localStorage.setItem(`user_${tutor._id}_status`, updatedStatus ? "Blocked" : "Unblocked");
  ...
  return (
    <>
     <div className="w-screen h-screen fixed   px-20  py-11 bg-gradient-to-br from-white">
      {/* component */}
      <table className="border-collapse w-full">
        ...
        {/* Table Body */}
        <tbody>
          {/* Map through paginated data and render rows */}
          {paginatedData.map((tutor) => (
            ...
            >
              ...
              <td className="w-full lg:w-auto p-3 text-sky-800 border border-b text-center block lg:table-cell relative lg:static">
                <button
                  onClick={() => userStatus(tutor)}
                  className="text-white bg-sky-500 font-bold hover:text-blue-600 py-2 px-4 rounded"
                >
